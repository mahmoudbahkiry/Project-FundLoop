...
interface AuthContextType {
  ...
  completeRegistration: (uid: string, userData: any) => Promise<void>;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const [isLoginAttemptInProgress, setIsLoginAttemptInProgress] =
    useState(false);
  ...
  useEffect(() => {
    ...
    const unsubscribe = setupAuthStateListener(async (firebaseUser) => {
      if (firebaseUser) {
        try {
          console.log(
            "Auth state changed: User is signed in with UID:",
            firebaseUser.uid
          );
  ...
  const signUp = async (
    ...
    try {
      ...
      // Create user object with basic auth data only
      const user: User = {
        uid: firebaseUser.uid,
        firstName,
        lastName,
      ...
      // Store minimal user data in AsyncStorage to track auth state
      await AsyncStorage.setItem("user", JSON.stringify(user));
      setUser(user);

      console.log("Basic user authentication completed successfully");

      // Return the UID to be used in the complete registration step
  ...
  const completeRegistration = async (uid: string, userData: any) => {
    try {
      setLoading(true);
      console.log("Completing registration for user:", uid);
      console.log("Saving complete user data to Firestore:", userData);
      ...
      await saveUserData({
        ...
        tradingExperience: userData.tradingExperience,
        authType: "email",
        // Add additional fields from financial information
      ...
      // Store complete user in AsyncStorage
      const userForStorage = {
        ...completeUser,
        // Convert Date to ISO string for storage
        dateOfBirth: completeUser.dateOfBirth
          ? completeUser.dateOfBirth.toISOString()
          : undefined,
      };
      ...
      console.log("Registration completed and data saved to Firestore");
    } catch (error) {
